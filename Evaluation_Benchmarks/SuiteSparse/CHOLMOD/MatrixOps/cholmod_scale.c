...
)
{
   ...
   gettimeofday(&end, NULL);
    ...
    totaltime = (end.tv_sec+ (double)end.tv_usec/1000000) -  (start.tv_sec + (double)start.tv_usec/1000000);

    printf("Time taken by the scaling routine = %lf s\n\n", totaltime);
	...
	#pragma omp parallel for private(j,p,t,pend)num_threads(CHOLMOD_OMP_NUM_THREADS)schedule(dynamic)
	for (j = 0 ; j < ncol ; j++)
	{
	    t = s [j] ;
	    p = Ap [j] ;
